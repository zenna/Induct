...
\citep{Dearden98bayesianq-learning}
...
\subsection*{Stochastic Javascript Implementation}
...
Code snippet \ref{fig:poissonRnd} demonstrates the implementation of an Elementary Random Primitive of a discrete Poisson distribution and is equivalent to the \verb+(poisson lambda)+ function found in Church.
...
\lstset{language=C}
\begin{lstlisting}[caption={A random ERP for Poisson distributions},label=poissonRnd]
...
\subsection*{Conitional Inference}
To support conditional inference Church generalises evaluation to \textit{conditional simulation} \cite{VIKASH} of any generative process, organising computation around a procedure \verb+(QUERY <expression> <environment> <predicate>)+.
...
\verb+QUERY+ expressions in Church Code snippet \ref{fig:query} demonstrates a simple rejection sampling implementation of query.
